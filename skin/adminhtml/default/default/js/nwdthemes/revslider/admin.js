...
UniteAdminRev = new function(){
	...
	********************************/
	...
	var openMageImageDialog = function(title, onInsert, isMultiple) {
			...
			new Ajax.Request(this.onInsertUrl, {
				...
				onSuccess: function(transport) {
					try {
						...
						onInsert(transport.responseText, "");
		...
		MediabrowserUtility.openDialog(revMageImageUploadUrl, false, false, title, {zIndex: zIndexMax});
	}
	...
	t.getUrlShowImage = function(imageURL,width,height,exact){
		...
		var urlImage = g_urlAjaxShowImage.replace('[image]', Base64.encode(imageURL));
	...
	var initVideoDialog = function(){
		...
		jQuery("#button_youtube_search").click(function(){
			...
			img.onload = function() {
				var img = jQuery(this);
				if (img[0].width <= 170 || img[0].height <= 140)
				{
					var imghq = new Image();
					imghq.onload = function() {
						...
						if (img[0].width <= 170 || img[0].height <= 140)
						{
							var imgmaxres = new Image();
							imgmaxres.onload = function() {
								UniteAdminRev.onYoutubeCallback(jQuery(this));
							}
							imghq.src = "https://img.youtube.com/vi/"+youtubeID+"/maxresdefault.jpg"
						}
						else
						{
					...
					imghq.src = "https://img.youtube.com/vi/"+youtubeID+"/hqdefault.jpg"
				}
				else
				{
					UniteAdminRev.onYoutubeCallback(img);
