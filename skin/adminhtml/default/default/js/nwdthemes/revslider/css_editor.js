...
UniteCssEditorRev = new function(){
	...
	var g_allGoogleFonts = [];
	var g_loadedGoogleFonts = [];
	...
	t.setCssPreviewLive = function(){
		if(cssExpertEditor){
			...
			loadGoogleFont(jQuery('input[name="css_font-family"]').val());
	...
	t.setCssStylesExpert = function(ignoreExample){
		...
		jQuery.each(curActiveStyles, function(_key, _value) {
			if (typeof _value == 'function' || _key.indexOf('function ') == 0)
			{
				delete curActiveStyles[_key];
			}
		});
	...
	var updateStylesInDb = function(handle, id){
		...
		jQuery.each(curFullClass, function(i, val){
			...
			jQuery.each(newObj, function(key, value) {
				if (typeof value == 'function' || key.indexOf('function ') == 0)
				{
					delete newObj[key];
				}
			});
	...
	/**
	 * Set google fonts list
	 */

	t.setGoogleFonts = function(fonts) {
		g_allGoogleFonts = fonts;
	}

	/**
	 * Preload google font on selection
	 */

	var loadGoogleFont = function(font) {
		if (g_allGoogleFonts.indexOf(font) != -1 && g_loadedGoogleFonts.indexOf(font) == -1)
		{
			var ss = document.createElement("link");
			ss.type = "text/css";
			ss.rel = "stylesheet";
			ss.href = "//fonts.googleapis.com/css?family=" + font;
			document.getElementsByTagName("head")[0].appendChild(ss);

			g_loadedGoogleFonts.push(font);
		}
	}
