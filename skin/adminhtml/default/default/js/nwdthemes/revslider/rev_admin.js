...
RevSliderAdmin = new function(){
		...
		var initSaveSliderButton = function(ajaxAction){
			...
			jQuery("#button_save_slider,#button_save_slider_top").click(function(){
				
					var postfix = jQuery(this).attr('id') == 'button_save_slider_top' ? '_top' : '';
					...
					if(ajaxAction == "update_slider"){
						...
						UniteAdminRev.setAjaxLoaderID("loader_update" + postfix);
						UniteAdminRev.setAjaxHideButtonID("button_save_slider" + postfix);
						UniteAdminRev.setSuccessMessageID("update_slider_success" + postfix);
					}
			...
			jQuery("#"+typeSettingName).change(function(){
				...
				jQuery(arrTypes).each(function(index,postType){
					...
					for(catIndex in objCats){
						...
						if (typeof objCats[catIndex] === 'object')
						{
							var catLabel = objCats[catIndex].label;
							var catValue = objCats[catIndex].value;

							//add option to cats select
							var opt = new Option(catLabel, catValue);

							if(catValue.indexOf("option_disabled") == 0)
						...
						else if (typeof objCats[catIndex] != 'function')
						{
		...
		var initSliderViewCommon = function(){
			...
			// not need it as we have single posts type
			updateCatByPostTypes("post_types","post_category");

			jQuery("input[name='source_type']").click(function(){ //check for post click
				
				if (jQuery(this).val() == 'posts' && ! jQuery('#post_types option:selected').length)
				{
					jQuery('#post_types option:first').prop('selected', true);
					jQuery('#post_types').trigger('change');
				}
				...
				if(jQuery(this).val() == 'posts' || jQuery(this).val() == 'specific_posts')
				{
					...
					jQuery('#min_height_row').hide();
				}
				else if ( ! g_sliderTemplate)
				{
					...
					//show more elements
					jQuery('#slider_type_row:not(.disabled)').show();
					jQuery('#slider_type_row:not(.disabled)').prev().show();
					jQuery('#fullscreen_offset_container_row:not(.disabled)').show();
					jQuery('#full_screen_align_force_row:not(.disabled)').show();
					jQuery('#slider_size_row:not(.disabled)').show();
					jQuery('#auto_height_row:not(.disabled)').show();
					if (jQuery('input[name=slider_type]:checked').val() == 'fullwidth' || jQuery('input[name=slider_type]:checked').val() == 'fullscreen') {
						jQuery('#force_full_width_row').show();
					}
					jQuery('#responsitive_row:not(.disabled)').show();
					jQuery('#responsitive_row:not(.disabled)').next().show();
				    jQuery('#min_height_row:not(.disabled)').show();
				}
				jQuery('input[name=slider_type]:checked').trigger('click');
			});
			
			jQuery("input[name='slider_type']").click(function(){
				if (jQuery("input[name='source_type']:checked").val() == 'gallery') {
						if(jQuery(this).val() == 'fullwidth' || jQuery(this).val() == 'fullscreen')
						{
								jQuery('#force_full_width_row').show();
						}
						else
			...
			jQuery(document).ready(function(){
				...
				if (jQuery('#source_type_2').is(':checked'))
				{
					jQuery('#source_type_1').trigger('click');
					jQuery('#source_type_2').trigger('click');;
				}
		...
		**/
		...
		this.initEditSliderView = function(){
			...
			jQuery("#button_delete_slider,#button_delete_slider_top").click(function(){
			...
			//preview slider actions
			jQuery("#button_preview_slider,#button_preview_slider_top").click(function(){
				var sliderID = jQuery("#sliderid").val();
				openPreviewSliderDialog(sliderID);
			});
			...
			jQuery(".export_slider_standalone").click(function(){
				...
				jQuery("#dialog_preview_sliders").dialog({
					...
					open:function(event,ui){
						...
						jQuery("#preview_slider_form_key").val(window.FORM_KEY);
		...
		var openPreviewSliderDialog = function(sliderID){
			...
			jQuery("#dialog_preview_sliders").dialog({
				...
				open:function(event,ui){
					...
					var $buttonPanel = jQuery(this).siblings('.ui-dialog-buttonpane');
					$buttonPanel.prepend( jQuery('#preview_slider_store_select_tempalte').html() );
					jQuery('.preview_slider_store_select select', $buttonPanel).on('change', function() {
						jQuery('#preview_slider_store_id').val( jQuery(this).val() );
						form1.submit();
				    });
				},
				close:function(){
				    jQuery('.preview_slider_store_select', jQuery(this).siblings('.ui-dialog-buttonpane') ).remove();
					var form1 = jQuery("#form_preview")[0];
					jQuery("#preview_sliderid").val("empty_output");
					jQuery("#preview_slider_markup").val('false');
					jQuery('#preview_slider_store_id').val('');
					form1.action = g_urlAjaxActions;
					form1.submit();
				}
		...
		var initSlideListGlobals = function(sliderID){
			...
			jQuery("#list_slides .icon_state:not(.disabled_control)").click(function(){
			...
			//change image
			jQuery(".col-image .slide_image:not(.disabled_control)").click(function(){
		...
		t.initSlidesListView = function(sliderID){
			...
			jQuery("#select_storeview").change(function() {
				document.location.href = g_storeViewLink.replace('[store_id]', jQuery(this).val());
			});
			...
			jQuery(".button_copy_slide").click(function(){
				...
				buttons[textUpdate] = function(){
					...
					var operation = jQuery('input[name=copy_move_operation]:checked').val();
		...
		var openPreviewSlideDialog = function(slideID,useParams){
			...
			jQuery("#dialog_preview").dialog({
					...
					open:function(event,ui){
						...
						jQuery("#preview_slide_form_key").val(window.FORM_KEY);
