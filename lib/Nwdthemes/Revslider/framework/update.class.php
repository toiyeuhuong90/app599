...
class UniteUpdateClassRev {
	...
	protected function _check_updates() {
		//reset saved options
		//Mage::getModel('nwdrevslider/options')->updateOption($this->option, false);

		// Get data
		if(empty($this->data)) {
			$data = Mage::getModel('nwdrevslider/options')->getOption($this->option, false);
		...
		$last_check = Mage::getModel('nwdrevslider/options')->getOption('revslider-update-check');
		if($last_check == false){ //first time called
			$last_check = time();
			Mage::getModel('nwdrevslider/options')->updateOption('revslider-update-check', $last_check);
		}
		...
		if(time() - $last_check > 172800){
			...
			if(isset($data->basic)) {
				Mage::getModel('nwdrevslider/options')->updateOption('revslider-update-check', time());

				$this->data->checked = time();
				$this->data->basic = $data->basic;
				$this->data->full = $data->full;

				Mage::getModel('nwdrevslider/options')->updateOption('revslider-latest-version', $data->full->version);
			}

		}

		// Save results
		Mage::getModel('nwdrevslider/options')->updateOption($this->option, $this->data);
	}
	...
	public function _retrieve_update_info() {
		...
		// Build request
		$api_key = Mage::getModel('nwdrevslider/options')->getOption('revslider-api-key', '');
		$username = Mage::getModel('nwdrevslider/options')->getOption('revslider-username', '');
		$code = Mage::getModel('nwdrevslider/options')->getOption('revslider-code', '');
	...
	public function _retrieve_version_info($force_check = false) {
		...
		$last_check = Mage::getModel('nwdrevslider/options')->getOption('revslider-update-check-short');
		if($last_check == false){ //first time called
			$last_check = time();
			Mage::getModel('nwdrevslider/options')->updateOption('revslider-update-check-short', $last_check);
		}
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			Mage::getModel('nwdrevslider/options')->updateOption('revslider-update-check-short', time());
			...
			if(isset($version_info->version)){
				Mage::getModel('nwdrevslider/options')->updateOption('revslider-latest-version', $version_info->version);
			}
		}
