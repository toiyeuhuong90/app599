<?php
	...
	class UniteSettingsRevProductRev extends UniteSettingsOutputRev{
		...
		protected function drawImageInput($setting){
			...
			if(!empty($value)){
				$pathImage = str_replace(Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA), '', $value);
				$urlThumb = Mage::helper('nwdrevslider/images')->resizeImg($pathImage, 100, 70);
				$img = '<div style="width:100px;height:70px;background:url('.$urlThumb.'); background-position:center center; background-size:cover;"></div>';
			}
			...
			?>
				...
				<input type="button" id="<?php echo $buttonID?>" style="width: 110px !important; float: left;" class='button-image-select button-primary revblue<?php echo $class?>' value="<?php echo Mage::helper('nwdrevslider')->__('Choose Image'); ?>"></input>
				<input type="button" class="button-image-remove button-primary revred" style="width: 110px !important;" id="<?php echo $buttonRemoveID; ?>" value="<?php echo Mage::helper('nwdrevslider')->__('Remove'); ?>" />
				<div class="clear"></div>
			<?php
		}
		...
		protected function drawSelectInput($setting){
			...
			foreach($setting["items"] as $value=>$text):
				...
				$selected = "";
				$addition = "";
				if(strpos($_value,"option_disabled") === 0){
					$addition = "disabled";
				}else{
					if(is_array($settingValue)){
						if(array_search($_value, $settingValue) !== false)
							$selected = 'selected="selected"';
					}else{
						if($_value == $settingValue)
							$selected = 'selected="selected"';
					}
				}
				?>
					<option <?php echo $addition?> value="<?php echo $_value?>" <?php echo $selected?>><?php echo $_text?></option>
				<?php
			endforeach
			?>
			</select>
