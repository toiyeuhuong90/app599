	...
	class UniteSettingsRev{
		...
		public function loadXMLFile($filepath){
			...
			foreach($fieldsets as $fieldset){
				...
				foreach($fields as $field){
					...
					$fieldLabel = Mage::helper('nwdrevslider')->__( (string)UniteFunctionsRev::getVal($attribs, "label") );
					...
					foreach($attribs as $key=>$value){
						...
						if ($key == 'description')
						{
							$value = Mage::helper('nwdrevslider')->__($value);
						}
					...
					foreach ($options as $_key => $_value) {
						$options[$_key] = Mage::helper('nwdrevslider')->__($_value);
					}
		...
		* Update values from post meta
		...
		public function updateValuesFromPostMeta($postID){
			...
			//Mage::helper('nwdrevslider')->dmp($postID);Mage::helper('nwdrevslider')->dmp($arrValues);exit();*/
