	...
	class RevSlide extends UniteElementsBaseRev{
		...
		public function initByData($record){
			...
			foreach ($layers as &$layer) {
				if ($layer["type"] == 'image')
				{
					$layer["image_url"] = $this->getFullImageUrl($layer["image_url"]);
				}				
			}
			
			$params['image'] = $this->getFullImageUrl(UniteFunctionsRev::getVal($params, "image"));
			$params['slide_thumb'] = $this->getFullImageUrl(UniteFunctionsRev::getVal($params, "slide_thumb"));

			$imageUrl = $params['image'];
			$this->imageThumb = $params['slide_thumb'];
		 ...
		 */
		private function normalizeLayers($arrLayers, $isUpdate = false){
			...
			foreach ($arrLayers as $key=>$layer){
				...
				//fix image urls
				if ($isUpdate && $type == 'image')
				{
					$layer["image_url"] = $this->getRelativeImageUrl($layer["image_url"]);					
				}
		 ...
		 * normalize params
		 */
		private function normalizeParams($params, $isUpdate = false){

			//init the id if absent
			$params["image_id"] = UniteFunctionsRev::getVal($params, "image_id");
			$params["image"] = UniteFunctionsRev::getVal($params, "image_url");

			unset($params["image_url"]);
			
			if ($isUpdate)
			{
				$params["image"] = $this->getRelativeImageUrl($params["image"]);
				if (isset($params['slide_thumb']))
				{
					$params['slide_thumb'] = $this->getRelativeImageUrl($params['slide_thumb']);
				}
			}
		...
		public function updateSlideFromData($data, $slideSettings){
			...
			if(gettype($layers) == "string"){
				$layersStrip = $layers;//stripslashes($layers);
			...
			$layers = $this->normalizeLayers($layers, true);
		 ...
		 * @return string
		 */
		public function getFullImageUrl($imageUrl = '') {
			if($imageUrl && strpos($imageUrl, 'http') !== 0)
			{
				$imageUrl = Mage::app()->getStore()->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA, Mage::helper('nwdrevslider')->isSsl() ) . Nwdthemes_Revslider_Helper_Images::IMAGE_DIR . '/' . $imageUrl;
			}
			return $imageUrl;
		}
		
		/**
		 * Generate relative image urls
		 *
		 * @param string $imageUrl
		 * @return string
		 */
		public function getRelativeImageUrl($imageUrl = '') {
			$folderURL = Mage::app()->getStore()->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA, Mage::helper('nwdrevslider')->isSsl() ) . Nwdthemes_Revslider_Helper_Images::IMAGE_DIR . '/';
			$imageUrl = str_replace($folderURL, '', $imageUrl);					
			return $imageUrl;
		}
