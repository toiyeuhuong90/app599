<?php
	...
	class RevSliderAdminOriginal extends UniteBaseAdminClassRev{
		...
		private function init(){
			...
			$validated = Mage::getModel('nwdrevslider/options')->getOption('revslider-valid', 'false');
			$notice = Mage::getModel('nwdrevslider/options')->getOption('revslider-valid-notice', 'true');
			...
			if(!$revSliderAsTheme){
				...
				if(Mage::getModel('nwdrevslider/options')->getOption('revslider-valid', 'false') === 'true') {
		...
		public static function createTable($tableName){
			...
			$checkForTablesOneTime = Mage::getModel('nwdrevslider/options')->getOption('revslider_checktables', '0');

			if($checkForTablesOneTime == '0'){
				Mage::getModel('nwdrevslider/options')->updateOption('revslider_checktables', '1');
		...
		private static function importSliderHandle($viewBack = null, $updateAnim = true, $updateStatic = true){
			...
			Mage::helper('nwdrevslider')->dmp(__("importing slider setings and data...",REVSLIDER_TEXTDOMAIN));
			...
			if($response["success"] == false){
				...
				Mage::helper('nwdrevslider')->dmp(__("Slider Import Success, redirecting...",REVSLIDER_TEXTDOMAIN));
