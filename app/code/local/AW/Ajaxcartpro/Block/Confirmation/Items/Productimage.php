...
class AW_Ajaxcartpro_Block_Confirmation_Items_Productimage extends Mage_Core_Block_Template
{
    ...
    protected function _toHtml()
    {
        ...
        $image = $helper->init($product, 'small_image')->resize($resize);
        
        if($postData = Mage::app()->getRequest()->getParam('actionData')){
            $actionData = Mage::helper('core')->jsonDecode($postData);
            if($actionData['added_quote_item']){
                $image = $actionData['added_quote_item'];
            }
        }
        ...
        $img = '<img src="' . $image .
               ...
               '" style="width:' . $resize .
               'px" height="' . $resize .
               '" /><input id="current_product" type="hidden" value="'.$product->getId().'">';
        return $img;
    }
}