<?php
class QSoft_Onestepcheckout_PaypalController extends Mage_Core_Controller_Front_Action{
	...
	protected  function _getHelper() {
		return Mage::helper ( 'onestepcheckout/paypal' );
	}
	...
	public function returnAction() {
		...
		$paypalCustomerData = Mage::getModel ( 'onestepcheckout/paypal_customer' )->getPaypalCustomerDataByField ( 'payer_id', $profile->getUserId() );
		...
		if (! $paypalCustomerData) {
			...
			if ($this->_getSession ()->isLoggedIn ()) {
				...
				$url = Mage::getUrl ( 'onestepcheckout/paypal/askLink' );
			...
			$paypalCustomer = Mage::getModel ( 'onestepcheckout/paypal_customer' );
				...
				try {
					...
					switch ($e->getCode ()) {
						...
						case Mage_Customer_Model_Customer::EXCEPTION_INVALID_EMAIL_OR_PASSWORD :
							...
							$redirectUrl = 'onestepcheckout/paypal/askLink';
							break;
						default :
							$message = $e->getMessage ();
					}
	...
	public function unlinkAction() {
		if ($this->_getSession ()->getCustomer ()->getId ()) {
			$paypalCustomer = Mage::getModel ( 'onestepcheckout/paypal_customer' )->unlinkAccount ();
	...
	public function askLinkPostAction() {
		...
		$askLinkUrl = 'onestepcheckout/paypal/askLink';
		...
		if (array_key_exists ( 'send', $params )) {
			...
			$isPaypalCustomerExists = Mage::getModel ( 'onestepcheckout/paypal_customer' )->isPaypalCustomerExists ( 'customer_id', $customerId );
			...
			if ($url == 'onestepcheckout/paypal/askLink') {
				$this->_redirect ( $url );
				return;
			}
