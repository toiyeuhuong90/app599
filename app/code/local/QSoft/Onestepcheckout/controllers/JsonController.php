<?php
class QSoft_Onestepcheckout_JsonController extends Mage_Core_Controller_Front_Action{
	...
	protected function _getShippingMethodsHtml(){
		...
		$shippingMethods->setTemplate('qsoft/onestepcheckout/onepage/shipping_method.phtml');
	...
	protected function _getPaymentMethodsHtml($use_method = false, $just_save = false){
		...
		else
		{
			...
			{
				$apply_method = Mage::helper('onestepcheckout')->getSelectedPaymentMethod();
	...
	public function saveBillingAction(){
		...
		if ($this->getRequest()->isPost()) {
			...
			$methods_before = Mage::helper('onestepcheckout')->getAvailablePaymentMethods();
			///////
			...
			if (!isset($result['error'])) {
				...
				$methods_after = Mage::helper('onestepcheckout')->getAvailablePaymentMethods();
				///////
				
				// check if need to reload payment methods
				$use_method = Mage::helper('onestepcheckout')->checkUpdatedPaymentMethods($methods_before, $methods_after);
		...
		if ($this->getRequest()->isPost()) {
			...
			*/
			if(!$result) {
				...
				$responseData['grandTotal'] = Mage::helper('onestepcheckout')->getGrandTotal();
	...
	public function savePaymentAction()
	{
		...
		try {
			...
			if (empty($result['error'])) {
				...
				$result['grandTotal'] = Mage::helper('onestepcheckout')->getGrandTotal();
			}
			if ($redirectUrl) {
	...
	*/
	public function saveOrderAction(){
		...
		try {
			...
			// save comments
			if (Mage::helper('onestepcheckout')->isShowComment())
			{
